...
public class Lab07c {
	...
	public static void main(String[] args) {
      ...
      System.out.println("The " + num + "th word is: " + getWord(str, num));
	}
    ...
    // Given a String, reutnr the first word in the String
    private static String getWord(String input, int num) {
        ...
        int index = 0;
        while (wordCount <= num && index < input.length()) {
            if (onWord == false && input.charAt(index) != ' ') {
                onWord = true;
                start = index;
            }
            else if (onWord == true && input.charAt(index) == ' ') {
                onWord = false;
                wordCount++;
            }
            if (wordCount == num) {
        ...
        return findWord? input.substring(start, index): "";
    }
}
